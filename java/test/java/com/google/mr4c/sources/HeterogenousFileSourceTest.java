...
public class HeterogenousFileSourceTest {
	...
	private FileSource buildSource() throws IOException {
		Map<String,URI> map = new HashMap<String,URI>();
		map.put("file1", URI.create("output/data/sources/hetero/file1"));
		map.put("file2", HadoopTestUtils.toTestDFSURI("/test/sources/hetero"));
		map.put("file3", URI.create("output/data/sources/hetero/file3"));
		FileSource src = new HeterogenousFileSource(map);
		src.ensureExists();
		src.clear();
		return src;
	}
	...
	@Test public void testCopy() throws Exception {
		FileSource copyFrom = buildSource();
		SourceTestUtils.populateSource(copyFrom);
		SourceTestUtils.testCopySource(copyFrom, m_src, true);
	}
