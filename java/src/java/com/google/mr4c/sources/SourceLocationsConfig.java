...
public class SourceLocationsConfig {

	protected static final Logger s_log = MR4CLogging.getLogger(SourceLocationsConfig.class);

	public static LocationsConfig load(ConfigDescriptor descriptor) throws IOException {
	...
	private static LocationsConfig loadFromFile(ConfigDescriptor descriptor) throws IOException {
		s_log.info("Loading locations config from [{}]", descriptor.getConfigFile());
		return loadFromContent(descriptor);
	}

	private static LocationsConfig loadInline(ConfigDescriptor descriptor) throws IOException {
		s_log.info("Locations config is inline");
		return loadFromContent(descriptor);
	}

	private static LocationsConfig loadFromContent(ConfigDescriptor descriptor) throws IOException {
	...
	public static LocationsConfig load(Reader reader) throws IOException {
		...
		return ser.deserializeLocationsConfig(reader);
	}
